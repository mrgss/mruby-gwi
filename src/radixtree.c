...
static size_t
node_size(rt_node *node)
{
 if (!node) return 0;
 size_t self_size, children_size, i;
 self_size = sizeof(*node) + node->klen + node->lalloc * (sizeof(rt_node) + sizeof(void*));
 children_size = 0;
 for (i = 0; i < node->lcnt; ++i) {
   children_size += node_size(node->leaf[i]);
 }
 return self_size + children_size;
}

size_t 
rt_tree_memory(rt_tree *t)
{
 size_t total;
 total = sizeof(*t) + node_size(t->root);
}